...
**Pas téléchargé — les photos.** Elles sont hébergées par les cinq sites
d'origine. Le moteur ne pointe **jamais** vers leurs serveurs : tant qu'aucune
photo n'a été déposée, la fiche affiche une vignette dessinée à partir du nom
du produit. `outils/images.py` fait le téléchargement et le redimensionnement,
il est complet et il fonctionne — mais il refuse de démarrer sans
`--je-detiens-les-droits`, parce que c'est une décision qui engage
l'exploitant du site, pas l'auteur du code.
...
`photos/` : relancer une correction du moteur ne perd ni les commandes reçues
ni les photos déposées.
...
- **Les photos** : lesquelles vous avez le droit de publier.
